...
class SortedCumugram(Listogram):

    def __init__(self, word_list=None):
        super(SortedCumugram, self).__init__()
        self.types = 0
        self.tokens = 0
        if word_list is not None:
            word_list.sort()
    ...
    def add_count(self, word, count=1):
        ...
        if len(self) == 0:
            ...
            self.append((word, self[-1][1]+count))
            self.types += 1
        elif word == self[-1][0]:
            self[-1] = (word, self[-1][1]+count)
        else:
            ...
            if self[target_index][0] == word:
                ...
                #         self[i] = (self[i][0], self[i][1]+count)
                # else:
                    # self = self[:target_index]+[i[1]+count for i in self[target_index:]]
                for i in range(target_index, self.types):
                    self[i] = (self[i][0], self[i][1]+count)
            else:
                self.types += 1
                if target_index == 0:
                    self.insert(target_index, (word, count))
                    for i in range(target_index+1, self.types):
                        self[i] = (self[i][0], self[i][1]+count)
                else:
                    self.insert(target_index, (word, self[target_index-1][1]+count))
                    for i in range(target_index+1, self.types):
                        self[i] = (self[i][0], self[i][1]+count)
        self.tokens += count
